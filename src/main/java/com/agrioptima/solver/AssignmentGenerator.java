/* AssignmentGenerator.java is useful because it tell does a provided crop asignment is valid or not.
    ...
    it uses a recursive backtracking approach to build up assignments one plot at a time,
...
public class AssignmentGenerator {
    ...
    // getValidAssignments() generates and returns a list of all valid crop
    // assignments for the given FarmState.
    // It initializes a working array for the current assignment and calls the
    // recursive generate() method to fill it out.
    ...
    // generate() is a recursive helper method that builds assignments one plot at a
    // time.
    // applying constraints at each step to prune invalid branches early.
    ...
    // rawCombinationCount() returns the total number of possible assignments
    // without any constraints, which is cropList.size()^numPlots.
    ...
    // valudCount() returns the number of valid assignments for a given state,by
    // calling getValidAssignments and returning its size.
    public int validCount(FarmState state) {
        return getValidAssignments(state).size();
    }

    // formatAssignment() formats an assignment into a human-readable string for
    // logging and demos.
