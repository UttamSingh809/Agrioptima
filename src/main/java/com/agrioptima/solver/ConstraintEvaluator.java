...
public class ConstraintEvaluator {
    private static final int NO_CROP = -1; // NO_CROP flag to indicate(-1) no previous crop planted on a plot (for
                                           // rotation checks)
    ...
    public ConstraintEvaluator() {
        this(2000.0, 500.0);// Default weights chosen to make water violations very costly, while still
                            // rewarding soil improvements.
    }
    ...
    /*
     * isValid() checks wheather a proposed crop plan (assignment) is legally
     * acceptable for the current farm state.
    ...
    public boolean checkWaterLimit(FarmState state, int[] assignment, List<Crop> cropList) {
        ...
        for (int i = 0; i < assignment.length; i++) {
            totalWaterNeeded += cropList.get(assignment[i]).getWaterPerAcre(); // sum up water needs for all assigned
                                                                               // crops
        }
        return totalWaterNeeded <= state.getRemainingWater(); // valid if total water needed is within the remaining
                                                              // water budget
    }
     ...
     * where adjustment is: - (waterPenaltyWeight * waterOveruse) +
     * (soilRewardWeight * totalSoilGain)
     */
    ...
    public double getWaterPenaltyWeight() {
        return waterPenaltyWeight;
    }

    public double getSoilRewardWeight() {
        return soilRewardWeight;
    }
}