...
public class DPSolver {
    ...
    // memo.get(state) = best total profit achievable from that state to the end.
    // Populated lazily during the dp() recursion.
    private final Map<FarmState, int[]> backpointer; // Backpointer store.
    // backpointer.get(state) = the crop assignment that leads to the best profit
    // from that state, Used by PlanReconstructor to rebuild the full seasonal
    // schedule.
    ...
    public DPSolver(StateManager stateManager, AssignmentGenerator generator, ConstraintEvaluator evaluator,
            List<Crop> cropList) {
    ...
    // Computes the maximum total profit achievable from the given state to the end
    ...
    private double dp(FarmState state) {
        ...
        statesExplored++;
        List<int[]> validAssignments = generator.getValidAssignments(state); // in this list i have passed rotation,
        ...
        // Store result in memo and backpointer, memo: so future visits to this state
        // return instantly and backpointer:so PlanReconstructor can trace the optimal
        // path
    ...
    // OPTIONAL: printStats() a fun build just to showcase DP solver work in
    // AgriOptima if UI is not prepared.
